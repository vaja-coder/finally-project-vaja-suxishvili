...
def see_info(item_pin):
    selected_item = Item.query.get(item_pin)
    return render_template("see_info.html", item=selected_item)
...
def add_items():
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        return redirect("/")
    return render_template("edd_item.html", form=form)

@app.route("/edit_item/<int:pin>", methods=["GET", "POST"])
def edit_items(pin):
    select_item = Item.query.get(pin)
    form = AddItemForm(
        item_name=select_item.name,
        img=select_item.img,
        size=select_item.size,
...
@app.route("/buy/<pin>", methods=["GET", "POST"])
def buy(pin):
    form = BuyingForm()
    if request.method == "POST":
